...
func (cb *ContextBuilder) Build() (BashibleContextData, map[string][]byte, map[string]error) {
	...
	for _, bundle := range cb.clusterInputData.AllowedBundles {
		for _, ng := range cb.clusterInputData.NodeGroups {
			...
			bashibleContext, err := cb.newBashibleContext(checksumCollector, bundle, ng, cb.clusterInputData.Template, cb.clusterInputData.APIServerEndpoints, cb.versionMap, &bundleNgContext)
			if err != nil {
...
func (cb *ContextBuilder) newBashibleContext(checksumCollector hash.Hash, bundle string, ng nodeGroup, template interface{}, clusterMasterAddresses []string, versionMap map[string]interface{}, bundleNgContext *bundleNGContext) (bashibleContext, error) {
	bc := bashibleContext{
